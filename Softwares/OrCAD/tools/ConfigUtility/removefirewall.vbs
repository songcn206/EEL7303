...
if quiet <> "-quiet" then
	...
	nResult = msgbox ("Click OK to remove firewall exceptions for OrCAD 16.6 Lite or Cancel to exit.", 65, "Firewall Exceptions Removal")
...
Next
if quiet <> "-quiet" then
	msgbox "Firewall exceptions removed."
end if
...
Function ReplaceTest(str1)
  ...
  for each x in OnlyFileName
  	if x <> "" then
	 if version > "6" then
	 	'msgbox eShell & "\system32\netsh.exe advfirewall firewall delete rule name=" & chr(34) & x & " (Release 16.6 Lite)" & chr(34) & " dir=in profile=any", 0
	 	wShell.run eShell & "\system32\netsh.exe advfirewall firewall delete rule name=" & chr(34) & x & " (Release 16.6 Lite)" & chr(34) & " dir=in profile=any", 0, TRUE
	 else
	 	'msgbox eShell & "\system32\netsh.exe firewall delete allowedprogram program=" & chr(34) & ReplaceTest & chr(34) & " profile=ALL", 0
	 	wShell.run eShell & "\system32\netsh.exe firewall delete allowedprogram program=" & chr(34) & ReplaceTest & chr(34) & " profile=ALL", 0, TRUE
	 end if
	end if
  next
